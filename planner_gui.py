...
class Task_Frame(ttk.Frame):
    def __init__(self, master, task_id, added, **kwargs):
        ...
        if added == True:
            ...
            self.task_deadline_placeholder.pack(pady=5)
            self.task_deadline_placeholder.place(x=1050,y=650)
...
class Adding_Configuration:
    def __init__(self, root, sanity_bar, sanity_percent, shared_queue, box_in_sanity_2):
        ...
        self.grid_frame.place(x=1048, y=200)
    ...
    def add_task(self):
        ...
        added = True
        frame_add = Task_Frame(self.grid_frame, task_id, added, title=frame.task_text_input_extract, desc =frame.task_desc_input_extract, deadline=frame.task_deadline_input_extract, diff=frame.task_diff_slider_extract)

        global removal_btn_Y
        removal_btn = ttk.Button(self.root, text=f"-T{task_id}", command=(lambda: [self.removal_update(task_id, self.task_frames), complete_btn.destroy(), removal_btn.destroy(), add_time_btn.destroy()]))
        removal_btn.config(width=5)
        removal_btn.pack(pady=5)
        removal_btn.place(x=1820, y=removal_btn_Y)
        removal_btn_Y +=52

        global complete_btn_Y
        complete_btn = ttk.Button(self.root, text=f"+T{task_id}", command=(lambda: [self.removal_update(task_id, self.task_frames), self.add_coins(diff=frame.task_diff_slider_extract), self.add_sanity(diff=frame.task_diff_slider_extract), complete_btn.destroy(), removal_btn.destroy(), add_time_btn.destroy()]))
        complete_btn.config(width=5)
        complete_btn.pack(pady=5)
        complete_btn.place(x=960, y=complete_btn_Y)
        complete_btn_Y +=52

        global add_time
        global add_time_btn_Y
        add_time_btn = ttk.Button(self.root, text=f"+1 Min", command=lambda: self.add_time_func(frame_add))

        add_time_btn.config(width=6)
        add_time_btn.pack(pady=5)
        add_time_btn.place(x=1690, y=add_time_btn_Y)
        add_time_btn_Y +=52

        row = len(self.task_frames) + 1
        col = 1
        frame_add.grid(row=row, column=col, padx=5, pady=5, sticky="e")
        self.task_frames[task_id] = frame_add
        self.add_btn.pack(pady=5)
        self.add_btn.place(x=1690, y=90)

    def removal_update(self, task_id, task_frames):
        global thread_event
        global thread_object
